...
if(isset($_POST['submit'])){
	if(!isset($_POST["fullname"]) || !isset($_POST["emailid"]) || !isset($_POST["contactno"]) || !isset($_POST["billingaddress"]) || !isset($_POST["shippingaddress"]) || !isset($_POST["password"]))
	{
		$error = "<p style='color:red'><i>*Please fill all information</i></p>";
	}
	else
	{
		if($_POST["fullname"] != "" && $_POST["emailid"] != "" && $_POST["contactno"] != "" && $_POST["billingaddress"] != "" && $_POST["shippingaddress"] != "" && $_POST["password"] != "")
		{
			...
			$sqlQuery = "INSERT INTO users (name, email, contactNo, password, shippingAddress, billingAddress, regDate) VALUES('".$_POST["fullname"]."', '".$_POST["emailid"]."', '".$_POST["contactno"]."', '".$_POST["password"]."', '".$_POST["shippingaddress"]."', '".$_POST["billingaddress"]."','$timestamp')";
			...
			if (mysqli_query($dbConnection, $sqlQuery)) {
				...
				$sqlQuery = "INSERT INTO userlog (userEmail, userIP, loginTime, status) VALUES('".$_SESSION["emailid"]."', '$ip', '$timestamp', 1)";
				mysqli_query($dbConnection, $sqlQuery);
				mysqli_close($dbConnection);
				header("Location: ./my-cart.php");
				exit;
			} else {
				echo "User Could not be added: " . $sql . "<br>" . mysqli_error($dbConnection);
			}
	...
	else
	{
		if($_POST["email"] != "" && $_POST["password"] != "")
		{
			...
			if (mysqli_query($dbConnection, $sqlQuery)) {
				...
				if($rowCount == 0){
					$error="<p style='color:red'><i>No such user</i></p>";
				} else {
					$row = mysqli_fetch_row($result);
					if($row[4] == $_POST["password"]){
						$_SESSION["userid"] = $row[0];
						$_SESSION["fullname"] = $row[1];
						$_SESSION["emailid"] = $row[2];
						$_SESSION["contactno"] = $row[3];
						$_SESSION["billingaddress"] = $row[6];
						$_SESSION["shippingaddress"] = $row[5];
						$_SESSION["login"] = true;
						$timestamp = date('Y-m-d H:i:s');
						$ip = inet_pton($_SERVER["REMOTE_ADDR"]);
						$sqlQuery = "INSERT INTO userlog (userEmail, userIP, loginTime, status) VALUES('".$_SESSION["emailid"]."', '$ip', '$timestamp', 1)";
						...
						$error="<p style='color:red'><i>Wrong password</i></p>";
					}
